...
async def test_service_2_down_triggers_unresolved_and_replay():
    ...
    try:
        # 1. Point to offline endpoint to simulate Service 2 down
        settings.resolution_url = "http://127.0.0.1:59999"
    ...
    assert resolved_matching[0]["resolved_verdict"] in [
        Verdict.BLOCK_IP.value, Verdict.QUARANTINE.value, Verdict.ALLOW.value, Verdict.MONITOR.value
    ]
